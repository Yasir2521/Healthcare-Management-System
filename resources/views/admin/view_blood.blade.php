...
<div class="container">
  <table>
    <thead>
      <tr>
        <th>Blood Donor's Name</th>
        <th>Blood Donor's Email</th>
        <th>Blood Type</th>
        <th>Blood Donor's Phone</th>
    ...
    @foreach ($bloods as $blood)
        <tr>
          ...
          <td><form action="{{ url('/del_blood') }}" method="POST">
    @csrf
    <input type="hidden" name="id" value="{{ $blood->id }}">
    <button type="submit" class="take-appointment-link" onclick="return confirm('Are you sure you want to remove the blood donor?')">Remove Deliveryman</button>
</form>
</td>
        </tr>
      @endforeach
